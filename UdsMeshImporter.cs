...
class UdsMeshImporter
{
    ...
    static public Mesh LoadMesh(string file)
    {
        var mesh = new Mesh();
        ...
        using (var ms = new System.IO.MemoryStream(buffer))
        {
            using (BinaryReader reader = new BinaryReader(ms))
            {
                ...
                if (!didFindMarker)
                {
                    throw new Exception("Couldn't find marker " + markerStr + " in file " + file);
                }
                ...
                Dictionary<int, int> indexRemap = new Dictionary<int, int>();
                {
                    ...
                    {
                        Debug.Log(String.Format("UdsMeshImporter: Sanity check failed: File {0} has too many vertices ({1}, limit is {2}) -- returning empty mesh.", file, fileVertexCount, vertexLimit));
                        return mesh;
                    }
                ...
                for (uint i = 0; i < normalCount; ++i)
                {
                    ...
                    normals[i].y = 0 - reader.ReadSingle();
...

                {
                    ...
                    for (uint submeshIndex = 0; submeshIndex < submeshCount; ++submeshIndex)
                    {
                        var indices = cookedSubmeshIndices[(int)submeshIndex].ToArray();
                      //  Array.Reverse(indices);
                        mesh.SetIndices(indices, MeshTopology.Triangles, (int)submeshIndex);
                    }
