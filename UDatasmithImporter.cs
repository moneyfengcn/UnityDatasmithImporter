...
public class UDatasmithImporter
{
    ...
    class UdsStaticMesh
    {
        ...
        public static UdsStaticMesh FromNode(string file, XmlNode node)
        {
            ...
            var fullMeshPath = Path.Combine(Path.GetDirectoryName(file), m.filePath);
            //m.assetRef = (Mesh)UnityEditor.AssetDatabase.LoadAssetAtPath(fullMeshPath, typeof(Mesh));
            //if (!m.assetRef)
            //{
            //    Debug.Log(String.Format("StaticMesh: AssetDatabase.LoadAssetAtPath(\"{0}\") failed to return a mesh object", fullMeshPath));
            //}
            m.assetRef = UdsMeshImporter.LoadMesh(fullMeshPath);
    ...
    class UdsTexture
    {
        ...
        static private Texture2D LoadTextureFromFile(string file, string name, int texturemode)
        {
            ...
            result.wrapMode = (UnityEngine.TextureWrapMode)texturemode;//UnityEngine.TextureWrapMode.Repeat
        ...
        public static UdsTexture FromNode(string dirPath, XmlNode node)
        {
            ...
            */
            int texturemode = 0;
            int.TryParse(node.Attributes["texturemode"].Value, out texturemode);
            ...
            {
                Debug.Log(String.Format("Texture Reference \"{0}\" to IES light profile \"{1}\" cannot be resolved: IES light profile import is not implemented.", tex.name, tex.filePath));
                return null;
            }


            tex.fullyQualifiedPath = Path.Combine(dirPath, tex.filePath);

            if (File.Exists(tex.fullyQualifiedPath))
            {
                tex.assetRef = LoadTextureFromFile(tex.fullyQualifiedPath, tex.name, texturemode);
            }
            ...
            //{
            //    Debug.Log(String.Format("UdsTexture::FromNode: Asset does not exist at path \"{0}\"", tex.fullyQualifiedPath));
            //}
    ...
    class UdsMaterial
    {
        ...
        public static UdsMaterial FromNode(XmlNode node, Dictionary<String, UdsTexture> textureElements)
        {
            Debug.Assert(node.Name == "MasterMaterial");
            UdsMaterial m = new UdsMaterial();

            m.assetRef = new Material(Resources.Load<Material>("Standard")) { name = "MT_" + node.Attributes["label"].Value };// new Material(Shader.Find("Standard"));
            ...
            */
            ...
            foreach (XmlNode kvpNode in node.ChildNodes)
            {
                ...
                else if (keyName == "TextureScale")
                {
                    var cc = ParseKVPColor(kvpNode);
                    Vector2 uvScale = new Vector2(cc.r, cc.g);
                ...
                {
                    UdsTexture texRef;
                    if (textureElements.TryGetValue(kvpNode.Attributes["val"].Value, out texRef))
                    {
                        m.assetRef.mainTexture = texRef.assetRef;
                    }
                ...
                else if (keyName == "DiffuseMap")
                {
                    ...
                    if (texRef == null)
                    {
                        Debug.Log(String.Format("Missing diffuse texref \"{0}\" while assembling material node \"{1}\"", kvpNode.Attributes["val"].Value, m.name));
                    }
                    else
                    {
                ...
                else if (keyName == "BumpMap")
                {
                    ...
                    if (texRef == null)
                    {
                        Debug.Log(String.Format("Missing bump texref \"{0}\" while assembling material node \"{1}\"", kvpNode.Attributes["val"].Value, m.name));
                    }
                    else
                    {
                        Vector2 uvOffset, uvScale;
    ...
    private void ImportActorChildren(GameObject parentObject, XmlNode containerNode, Vector3 world)
    {
        ...
        foreach (XmlNode node in containerNode.ChildNodes)
        {
            ...
            if (node.Name == "ActorMesh")
            {
                ...
                if (meshNode != null)
                {
                    ...
                    if (mesh.assetRef)
                    {
                        ...
                        {
                            if (materialIdx >= mesh.materialRefs.Count)
                            {
                                Console.WriteLine("HH");
                                mats[materialIdx] = new Material(Resources.Load<Material>("Standard")) { name = "MT_" + mesh.name };
                            }
                            else
                            {
                        ...
                        // Process imported metadata and try to do something reasonable with it
                        {
                            ...
                            if (RevitLayer != null)
                            {
                                ...
                                {
                                    Debug.Log(String.Format("Unhandled Layer \"{0}\" -- ActorMesh \"{1}\" will not have physics and lighting behaviours automatically mapped", RevitLayer, objName));
                                }
                    ...
                    {
                        Debug.Log(String.Format("ActorMesh {0} mesh {1} assetRef is NULL", obj.name, meshName));
                    }
                }
            } // ActorMesh


            obj.transform.position = position;// - parentObject.transform.localPosition;
            ...
            if (node.Name == "Light" && m_ImportLights)
            {
                ...
                {
                    Debug.Log(String.Format("Light {0}: Unhandled \"type\" \"{1}\", defaulting to Point", objName, node.Attributes["type"].Value));
                }
            ...
            if (obj.name.StartsWith("Enscape_"))
            {
                //如果enscape的东西 就替换
                if (obj.name.StartsWith("Enscape_SpotLight_"))
                {
                    //添加点光
            ...
            { // children node processing
                ...
                if (childrenNode != null)
                {
                    ...
                    //obj.transform.InverseTransformPoint(obj.transform.position);
                    ImportActorChildren(obj, childrenNode, position);
                }
            } // children node processing
    ...
    public GameObject LoadModel(string file)
    {
        var dirPath = Path.GetDirectoryName(file);
        XmlDocument doc = new XmlDocument();
        {
            String wholeDoc = File.ReadAllText(file);
        ...
        {
            UdsTexture tex = UdsTexture.FromNode(dirPath, node);
            if (tex != null)
            {
        ...
        {
            UdsMaterial m = UdsMaterial.FromNode(node, textureElements);
        ...
        {
            UdsStaticMesh m = UdsStaticMesh.FromNode(file, node);
            ...
            for (int materialIdx = 0; materialIdx < m.materialNames.Count; ++materialIdx)
            {
                ...
                {
                    Debug.Log(String.Format("Can't resolve Material ref \"{0}\"", m.materialNames[materialIdx]));
                }
        ...
        ImportActorChildren(sceneRoot, rootElement, sceneRoot.transform.position);
